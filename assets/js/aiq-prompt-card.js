...
(function ($) {
  ...
  const AIQPromptCard = {
    ...
    init: function () {
      ...
      if (typeof elementorFrontend !== "undefined") {
        elementorFrontend.hooks.addAction(
          "frontend/element_ready/aiq_prompt_card.default",
          function ($scope) {
    ...
    handleCopyClick: function (e) {
      e.preventDefault();
      const $button = $(e.currentTarget);
      const promptText = $button
        .closest(".aiq-prompt-card")
        .find(".aiq-prompt-card__prompt")
        .text();
      ...
      this.copyToClipboard(variableName)
        .then(() => {
          ...
          this.announceForScreenReaders(
            aiqPromptCardConfig.i18n.variable_copied.replace(
              "%s",
              variableName,
            ),
          );
