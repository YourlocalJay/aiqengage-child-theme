...
(function ($) {
  ...
  $(window).on("elementor/frontend/init", function () {
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/aiq_comparison_matrix.default",
      initComparisonMatrix,
    );
  });
  ...
  function highlightDifferences($matrix) {
    const $rows = $matrix.find(
      ".aiq-comparison-matrix__row:not(.aiq-comparison-matrix__header):not(.aiq-comparison-matrix__row--category):not(.aiq-comparison-matrix__row--cta)",
    );

    $rows.each(function () {
      const $row = $(this);
      const $cells = $row.find(
        ".aiq-comparison-matrix__cell:not(.aiq-comparison-matrix__feature)",
      );
  ...
  function setupSorting($matrix) {
    ...
    const $sortControl = $(
      '<div class="aiq-comparison-matrix__sort-control"></div>',
    );
    const $sortLabel = $(
      '<span class="aiq-comparison-matrix__sort-label">Sort by: </span>',
    );
    const $sortSelect = $(
      '<select class="aiq-comparison-matrix__sort-select"></select>',
    );
    ...
    $rows.each(function () {
      const rowName = $(this)
        .find(".aiq-comparison-matrix__feature")
        .text()
        .trim();
      $sortSelect.append(
        `<option value="row-${$(this).index()}">${rowName}</option>`,
      );
    });
    ...
    $sortSelect.on("change", function () {
      ...
      $targetRow
        .find(".aiq-comparison-matrix__cell")
        .not(".aiq-comparison-matrix__feature")
        .each(function (colIndex) {
          ...
          if ($(this).find(".fa-check").length) {
            ...
            value = $(this)
              .find(".aiq-comparison-matrix__rating")
              .data("rating");
      ...
      reorderColumns(
        $matrix,
        columnValues.map((item) => item.index + 1),
      ); // +1 to account for feature column
    });
  }
  ...
  function reorderColumns($matrix, columnOrder) {
    ...
    $matrixTable
      .find(
        ".aiq-comparison-matrix__feature, .aiq-comparison-matrix__feature-header",
      )
      .css("order", "0");
  ...
  function setupResponsiveBehavior($matrix) {
    ...
    if ($table.width() > $wrapper.width()) {
      ...
      const $scrollLeft = $(
        '<div class="aiq-comparison-matrix__scroll-indicator aiq-comparison-matrix__scroll-indicator--left"><i class="fas fa-chevron-left"></i></div>',
      );
      const $scrollRight = $(
        '<div class="aiq-comparison-matrix__scroll-indicator aiq-comparison-matrix__scroll-indicator--right"><i class="fas fa-chevron-right"></i></div>',
      );
  ...
  function setupAccessibility($matrix) {
    ...
    $matrix
      .find(".aiq-comparison-matrix__cell")
      .not(".aiq-comparison-matrix__feature, .aiq-comparison-matrix__header")
      .attr("role", "cell");
    ...
    $matrix
      .find(".aiq-comparison-matrix__cell a, .aiq-comparison-matrix__cta")
      .attr("tabindex", "0");
