...
(function () {
  ...
  class ProgressBarManager {
    ...
    setupAccessibility(barData) {
      ...
      setInterval(() => {
        const current = parseInt(
          barData.element.getAttribute("aria-valuenow"),
          10,
        );
    ...
    shouldAnnounceProgress(current, lastAnnounced) {
      ...
      return (
        Math.floor(current / 25) > Math.floor(lastAnnounced / 25) ||
    ...
    updateSingleBar(barData, progress) {
      ...
      if (barData.percentageElement) {
        ...
        barData.percentageElement.classList.add(
          "aiq-progress-percentage-changing",
        );
        setTimeout(() => {
          barData.percentageElement.classList.remove(
            "aiq-progress-percentage-changing",
          );
        }, barData.speed);
      }
