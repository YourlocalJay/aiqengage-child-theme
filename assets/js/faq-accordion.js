...
(function () {
  ...
  function setupAccordion(accordion) {
    ...
    questions.forEach((question, index) => {
      ...
      question.addEventListener("keydown", (e) =>
        handleKeyboardNavigation(e, questions, index),
      );
    });

    // Announce accordion setup to screen readers
    announceToScreenReader(
      `FAQ accordion with ${questions.length} questions loaded. Use Tab to navigate questions, Enter or Space to expand, arrow keys to move between questions.`,
    );
  }
  ...
  function setupSearch(accordion, searchInput) {
    ...
    if (!instructions) {
      ...
      instructions.textContent =
        "Type to search through FAQ questions. Results will be filtered as you type.";
    ...
    const debouncedSearch = debounce(function () {
      ...
      items.forEach((item) => {
        const questionText =
          ...
          item
            .querySelector(CONFIG.selectors.question)
            .textContent.toLowerCase();
        const answerText = item
          .querySelector(CONFIG.selectors.answer)
          .textContent.toLowerCase();
        const isMatch =
          !searchTerm ||
      ...
      if (searchTerm) {
        announceToScreenReader(
          `${visibleCount} question${visibleCount !== 1 ? "s" : ""} found for "${searchTerm}"`,
        );
      }
    }, 300);
  ...
  function toggleItem(item, forceState) {
    ...
    const isOpening =
      forceState !== undefined
        ? forceState
        : !item.classList.contains(CONFIG.classes.active);
  ...
  function handleKeyboardNavigation(event, questions, currentIndex) {
    ...
    announceToScreenReader(
      `Question ${targetIndex + 1} of ${questions.length}: ${questions[targetIndex].textContent}`,
    );
