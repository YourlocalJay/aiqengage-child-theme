...
class Blueprint_Flow_Widget extends Widget_Base {
	...
	public function get_keywords() {
		return array( 'blueprint', 'flow', 'diagram', 'process', 'automation', 'node' );
	}
	...
	public function get_style_depends() {
		return array( 'aiqengage-child-blueprint-flow' );
	}
	...
	protected function register_controls() {
		...
		$roi_fields_repeater->add_control(
			...
			array(
				...
				'default'   => 0,
				'condition' => array(
					'field_type' => array( 'slider', 'percentage' ),
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'condition'  => array(
					'connector_style' => array( 'dashed', 'dotted' ),
				),
			)
		);
	...
	protected function render() {
		...
		<div <?php $this->print_render_attribute_string( 'wrapper' ); ?>>
			...
			<div class="aiq-blueprint-flow__nodes" role="list">
				<?php
				foreach ( $settings['nodes'] as $index => $node ) :
					++$node_count;
					...
					$this->add_render_attribute(
						...
						array(
							...
							'role'         => 'listitem',
							'aria-label'   => sprintf( __( 'Step %1$d: %2$s', 'aiqengage-child' ), $node_count, $node['node_title'] ),
					...
					<div <?php $this->print_render_attribute_string( $node_key ); ?>>
						...
						<?php if ( $node['node_icon_type'] === 'icon' && ! empty( $node['node_icon']['value'] ) ) : ?>
							<div class="aiq-blueprint-flow__node-icon">
								<?php \Elementor\Icons_Manager::render_icon( $node['node_icon'], array( 'aria-hidden' => 'true' ) ); ?>
							</div>
					...
					<?php
					if ( $settings['show_connectors'] === 'yes' && $index < count( $settings['nodes'] ) - 1 ) :
						$connector_classes = array(
			...
			<?php if ( $settings['show_roi_calculator'] === 'yes' ) : ?>
				<div class="aiq-blueprint-flow__roi-calculator">
					...
					<form class="aiq-blueprint-flow__roi-form" data-formula="<?php echo esc_attr( $settings['roi_formula'] ); ?>">
						<?php
						foreach ( $settings['roi_fields'] as $field_index => $field ) :
