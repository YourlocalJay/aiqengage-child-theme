...
(function (global, factory) {
	...
	function compileList(schema, name) {
		...
		schema[name].forEach(
			function (currentType) {
				...
				result.forEach(
					function (previousType, previousIndex) {
	...
	function readBlockScalar(state, nodeIndent) {
		...
		if (is_WHITE_SPACE( ch )) {
			...
			if (ch === 0x23/* # */) {
				do {
					ch = state.input.charCodeAt( ++state.position ); } while ( ! is_EOL( ch ) && (ch !== 0));
			}
		}
	...
	function readTagProperty(state) {
		...
		if (isVerbatim) {
			do {
				ch = state.input.charCodeAt( ++state.position ); } while (ch !== 0 && ch !== 0x3E/* > */);
	...
	function readDocument(state) {
		...
		while ((ch = state.input.charCodeAt( state.position )) !== 0) {
			...
			while (ch !== 0) {
				...
				if (ch === 0x23/* # */) {
					do {
						ch = state.input.charCodeAt( ++state.position ); } while (ch !== 0 && ! is_EOL( ch ));
					break;
				}
